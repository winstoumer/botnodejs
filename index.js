...
app.get('/api/miners/:telegramUserId', async (req, res) => {
  try {
    ...
    // Получаем список остальных майнеров, начиная с уровня майнера пользователя
    ...
    const miners = userMiner ? [userMiner, ...otherMinersResult.rows] : otherMinersResult.rows;
    res.json(miners);
  } catch (error) {
